...
def sudoku_master(img_original):
    ...
    if corners is not None:
        ...
        img_sudoku_final = perspective_transform(
            img_cropped_sudoku, transformation_matrix, original_shape
        )
...
def find_sudoku(img, draw_contours=False, test=False):
    ...
    edges = cv2.adaptiveThreshold(
        edges, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 19, 2
    )
        ...
        for cnt in conts:
            ...
            if len(cnt) > 3:
                ...
                for corner in corners:
                    if (
                        corner[0][0] in leftright_edges
                        or corner[0][1] in topbottom_edges
                    ):
                        badobject = True
...
def build_sudoku(sudoku_image, test=False):
    ...
    edges = cv2.adaptiveThreshold(
        edges, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 7, 7
    )
    ...
    for i in range(9):
        for j in range(9):
            ...
            contours, _ = cv2.findContours(
                fat_square, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
            )
            ...
            if contours:
                ...
                if cv2.contourArea(cnt) > minarea:
                    ...
                    if (
                        number_image is None
                        or number_image.shape[0] < 2
                        or number_image.shape[1] < 2
                    ):
